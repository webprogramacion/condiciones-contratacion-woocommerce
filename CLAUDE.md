...
Todo el código que se distribuye está en el directorio `condiciones-contratacion-woocommerce/`, que se llama igual que el slug. La raíz del repositorio solo contiene herramientas, documentación, planificación y CI. Las rutas de los archivos del plugin son, por tanto, `condiciones-contratacion-woocommerce/condiciones-contratacion-woocommerce.php`, `condiciones-contratacion-woocommerce/includes/…`, etc.
...
2. Actualizar la versión en **todos** estos sitios:
   - Cabecera `Version:` de `condiciones-contratacion-woocommerce/condiciones-contratacion-woocommerce.php`.
   - Constante de versión del plugin (p. ej. `CCWOO_VERSION`) en el mismo archivo.
   - `Stable tag:` de `condiciones-contratacion-woocommerce/readme.txt`.
3. Documentar los cambios en el changelog, en ambos archivos:
   ...
   - `condiciones-contratacion-woocommerce/readme.txt`: añadir la misma entrada en la sección `== Changelog ==` con el formato de WordPress.org (`= X.Y.Z =`).
...
- El zip de distribución se construye en CI comprimiendo el directorio `condiciones-contratacion-woocommerce/` tal cual (`zip -rq`), de modo que la carpeta raíz del zip es la que exige WordPress.org y el archivo se puede instalar directamente desde la release de GitHub.
...
- No hay lista de exclusiones (`.distignore` se eliminó). La regla es la inversa: **nada de desarrollo se crea dentro de `condiciones-contratacion-woocommerce/`**; tests, configs, tooling y documentación van en la raíz.
- `LICENSE` está duplicada a propósito: la de la raíz es para que GitHub detecte la licencia, la del directorio del plugin es la que viaja en el zip. Si se toca, hay que actualizar las dos.
