...
10. **Carga de la pestaña de ajustes**: `CCWOO_Settings_Page` se carga **dentro** del callback de `woocommerce_get_settings_pages`, no en `plugins_loaded`. `WC_Settings_Page` vive en `includes/admin/settings/`, una ruta que el autoloader de WooCommerce no resuelve (solo prueba `includes/class-wc-settings-page.php`), y WooCommerce la incluye a mano en `WC_Admin_Settings::get_settings_pages()` justo antes de disparar el filtro. Cargar antes nuestra subclase hacía que el `class_exists` de guarda fuese `false`, la clase no se definiera y el filtro provocara un fatal (`Class "CCWOO_Settings_Page" not found`) con pantalla en blanco. El callback comprueba ambas clases antes de instanciar.

11. **Depuración**: `CCWOO_Logger` escribe en el sistema de registros de WooCommerce (`wc_get_logger()`, origen `condiciones-contratacion`, con respaldo en `error_log()`); errores y advertencias siempre, informativos solo con `WP_DEBUG` o el filtro `ccwoo_enable_logging`. Se complementa con una sección «Diagnóstico» en los ajustes, avisos por consola en el JS de bloques y la captura de excepciones al inicializar los componentes, para que un fallo se vea como aviso en el admin en lugar de como pantalla en blanco. *Alternativa descartada*: un registro propio en archivo — WooCommerce ya ofrece visor, rotación y borrado en su pantalla de Estado.

12. **Prefijo del código**: `CCWOO_` / `ccwoo_` para clases, funciones, opciones, hooks y handles. WPCS rechaza prefijos de menos de cuatro caracteres (`WordPress.NamingConventions.PrefixAllGlobals.ShortPrefixPassed`) por riesgo de colisión, de modo que el `CCW_` inicial no era admisible para un plugin publicado.
